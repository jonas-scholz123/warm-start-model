# Adapted from https://github.com/thu-ml/DPM-Solver-v3
...
def model_wrapper(
    ...
    def noise_pred_fn(z_t, t_continuous, cond=None):
        if t_continuous.reshape((-1,)).shape[0] == 1:
            t_continuous = t_continuous.expand((z_t.shape[0]))
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model(z_t, t_input, None, **model_kwargs)
        else:
            output = model(z_t, t_input, cond, **model_kwargs)
        if model_type == "noise":
            return output
            ...
            dims = z_t.dim()
            return (z_t - expand_dims(alpha_t, dims) * output) / expand_dims(
                sigma_t, dims
            )
        elif model_type == "v":
            alpha_t, sigma_t = (
            ...
            dims = z_t.dim()
            return (
                expand_dims(alpha_t, dims) * output + expand_dims(sigma_t, dims) * z_t
            )
            ...
            # See: https://diffusionflow.github.io/ for an explanation.
            dims = z_t.dim()
            ...
            u = output
            return (u * alpha_t + z_t) / (alpha_t + sigma_t)
    ...
    assert model_type in ["noise", "x_start", "v", "flow_matching", "score"]
...
class DPM_Solver_v3:
    def __init__(
        ...
        if statistics_dir is None:
            degen_steps = 1000
            l = np.ones((degen_steps + 1, 1, 1, 1))
            s = np.zeros((degen_steps + 1, 1, 1, 1))
            b = np.zeros((degen_steps + 1, 1, 1, 1))
        else:
    ...
    def get_time_steps(self, skip_type, t_T, t_0, N, device):
        return get_time_steps(self.noise_schedule, skip_type, t_T, t_0, N, device)
...
class NoiseScheduleFlowMatch:
    """
    A custom noise schedule that matches the path of the user's FlowMatching model.
    The path is x_t = (1-t)*x_0 + t*x_1, which means:
    - alpha_t = t
    - sigma_t = 1 - t
    """

    def __init__(self):
        self.schedule = "continuous"
        # TODO: These should be double checked.
        self.T = 1.0 - 1e-5
        self.total_N = 1000  # A default number of steps

    def marginal_alpha(self, t):
        return (1.0 - t).to(torch.float64)
    ...
    def marginal_lambda(self, t):
        ...
        # Add small epsilon to avoid log(0) at the boundaries
        alpha_t = self.marginal_alpha(t).clamp(1e-9, 1.0)
        sigma_t = self.marginal_std(t).clamp(1e-9, 1.0)
        return torch.log(alpha_t) - torch.log(sigma_t)

    def inverse_lambda(self, lamb):
        return 1.0 / (1.0 + torch.exp(lamb.to(torch.float64)))
